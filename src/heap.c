...
#define HEAPSIZEMO 40000000
snode_mo* heap_mo[HEAPSIZEMO];
unsigned long int heapsize_mo = 0;

void percolatedown_mo(int hole, snode_mo* tmp) {
  int child;
  if (heapsize_mo != 0) {
    for (; 2 * hole <= heapsize_mo; hole = child) {
      child = 2 * hole;
      if (child != heapsize_mo && heap_mo[child + 1]->key < heap_mo[child]->key)
        ++child;
      if (heap_mo[child]->key < tmp->key) {
        heap_mo[hole] = heap_mo[child];
        heap_mo[hole]->heapindex = hole;
      }
      else
        break;
    }
...
void percolateup_mo(int hole, snode_mo* tmp) {
  if (heapsize_mo != 0) {
    for (; hole > 1 && tmp->key < heap_mo[hole / 2]->key; hole /= 2) {
      heap_mo[hole] = heap_mo[hole / 2];
      heap_mo[hole]->heapindex = hole;
    }
    heap_mo[hole] = tmp;
    heap_mo[hole]->heapindex = hole;
...
void percolateupordown_mo(int hole, snode_mo* tmp) {
  if (heapsize_mo != 0) {
    if (hole > 1 && heap_mo[hole / 2]->key > tmp->key)
      percolateup_mo(hole, tmp);
    else
      percolatedown_mo(hole, tmp);
  }
}

void insertheap_mo(snode_mo* thiscell) {
  if (thiscell->heapindex == 0)
    percolateup_mo(++heapsize_mo, thiscell);
  else
    percolateupordown_mo(thiscell->heapindex, heap_mo[thiscell->heapindex]);
}
...
void emptyheap_mo() {
  for (int i = 1; i <= heapsize_mo; ++i)
    heap_mo[i]->heapindex = 0;
  heapsize_mo = 0;
}
...
snode_mo* popheap_mo() {
  ...
  thiscell = heap_mo[1];
  thiscell->heapindex = 0;
  percolatedown_mo(1, heap_mo[heapsize_mo--]);
  return thiscell;
}
